...
async def generate_dynamic_preset_menu(ctx: commands.Context, data, username: str, starting_page: str, emoji_map: dict, page_generator):
    main_embed: discord.Embed = page_generator(ctx=ctx, data=data, username=username, page=starting_page)
    view: DynamicPresetMenuView = DynamicPresetMenuView(ctx=ctx, data=data, username=username, emoji_map=emoji_map, page_generator=page_generator)
...
class StaticPresetMenuView(discord.ui.View):
    def __init__(self, ctx, list_of_embeds, emoji_list: list, alternate_colours: bool):
        super().__init__()
        self.ctx: commands.Context = ctx
        self.page: int = 0
...
async def generate_static_preset_menu(ctx: commands.Context, list_of_embeds: list[discord.Embed], emoji_list: list[str], message_object: discord.Message=None, alternate_colours: bool=False):
    view: StaticPresetMenuView = StaticPresetMenuView(ctx=ctx, list_of_embeds=list_of_embeds, emoji_list=emoji_list, alternate_colours=alternate_colours)
    if message_object:
...
async def generate_static_scrolling_menu(ctx, list_of_embeds: list[discord.Embed]):
    view: StaticScrollingMenuView = StaticScrollingMenuView(ctx=ctx, list_of_embeds=list_of_embeds)
    view.message = await ctx.send(embed=list_of_embeds[0], view=view)
...
class DynamicScrollingMenuView(discord.ui.View):
    def __init__(self, ctx: commands.Context, data, page_generator):
        super().__init__()
        self.ctx: commands.Context = ctx
        self.data: list = data
        self.page: int = 1
...
async def generate_dynamic_scrolling_menu(ctx: commands.Context, data, page_generator):
    starting_embed: discord.Embed = await page_generator(ctx=ctx, data=data, page=1)
    view: DynamicScrollingMenuView = DynamicScrollingMenuView(ctx=ctx, data=data, page_generator=page_generator)
...
async def generate_option_picker(ctx: commands.Context, embed: discord.Embed, number_of_options: int):
    ...
    view: OptionPickerView = OptionPickerView(ctx=ctx, number_of_options=number_of_options)
